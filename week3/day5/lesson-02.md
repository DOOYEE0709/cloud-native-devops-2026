...
**실제 출력 (`kubectl -n week3 describe pod hello-pod`)**
```text
Node:    paperclip-week3-control-plane/172.18.0.2   ← 이 node의 kubelet이 실행
IP:      10.244.0.5                                 ← Pod IP (cluster 내부용)
Containers:
  nginx:
    Container ID: containerd://5c5c0777...          ← runtime이 containerd임이 보임 (3교시 연결)
    Image:        nginx:1.27
...
exec로 내부에서 HTTP 요청 (이 nginx:1.27엔 curl이 있어서 성공):
```text
...
Content-Length: 615
```
- 여기서 `127.0.0.1` = **Pod 내부에서 자기 자신**으로의 요청 (localhost). 아직 Service 없이 내부 확인만 한 것.

요청 후 logs 다시 보면 그 접근이 찍힘:
```text
$ kubectl -n week3 logs hello-pod
... nginx/1.27.5 ... start worker process ...           ← 기동 로그
127.0.0.1 - - [29/Jun/2026...] "HEAD / HTTP/1.1" 200 0 "-" "curl/7.88.1"   ← 방금 exec curl 요청이 access log에 남음
```
- 읽는 포인트:
  - **기동 로그**(docker-entrypoint, start worker process) = container가 정상 부팅.
  - 마지막 줄 `"HEAD / HTTP/1.1" 200` = 방금 보낸 `curl -I`(HEAD 요청)가 **access log에 기록** → 요청이 실제로 nginx까지 도달했음을 증거로 확인.
  - 즉 **exec(요청) → logs(흔적 확인)** 으로 "정말 살아서 응답하나"를 evidence로 남기는 흐름.
...
- ⚠️ 그리고 exec 수정은 **manifest엔 안 남으므로**, 응급 처치가 끝나면 **반드시 manifest에도 반영**해야 함. 안 그러면 Pod 재생성 시 변경이 사라지고, "왜 또 똑같은 장애?"가 됨. (2교시 exec 주의와 연결)
